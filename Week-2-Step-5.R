...
plot(err) #Most errors come from temperature and humidity

#And clean our geo units from inconsistencies
filtered <-  filter(z4, temperature > -30, temperature < 50,
                    pressure > 0, humidity >= 0)

#See the sample quantiles corresponding to the given probabilities:
quantile(filtered$temperature, probs=seq(0, 1, 0.25), names=TRUE, na.rm=FALSE)
quantile(filtered$humidity, probs=seq(0, 1, 0.25), names=TRUE, na.rm=FALSE)
quantile(filtered$pressure, probs=seq(0, 1, 0.25), names=TRUE, na.rm=FALSE)
...
# Step 5 Inspect and summarize the most important statistical characteristics of the set of time series subject to further analysis ----

# Testing for independence
ljung_box<-data.frame()

# If the p-value of the Ljung-Box test is <0.05, then we can reject the null hypothesis with 95% confidence
# The null hypothesis is that the data is independently distributed
# Testing the P1 values in all the clusters
for (i in 1:length(ts_list)){
  ljung_box[i,1]<-i
  ljung_box[i,2]<-Box.test(ts_list[[i]]$P1, type = c("Ljung-Box"))$p.value
}
names(ljung_box)<-c("Geo Unit", "P-value")
summary(ljung_box$`P-value`)
# P-values are very close to zero, so the null hypothesis is rejected:
# therefore the P1 variables in all clusters are not independent, i.e. there is autocorrelation
# 3 clusters should be further examined, as the tests returned NA for the p-values
