...
for (i in 1:max(cluster_df$clust_no)){
  na_count[i] <- as.data.frame(round((colSums(is.na(cluster_df[which(clust_no == i)]))/(lengths(cluster_df[which(clust_no == i)])))*100, digits = 2))
  colnames(na_count)[i] <- paste("perc_NAs_clust_", i)
}
